...
Sub GenerateTable( rs, pagesize )
  ...
  for i = 3 to rs.fields.count - 1
        ...
        Response.Write("<TD BGCOLOR=BLUE align=right><FONT FACE=ARIAL SIZE=1 COLOR=WHITE>" + LISTNAME + "</FONT></TD>")
        ELSE
        Response.Write("<TD BGCOLOR=BLUE align=right><FONT FACE=ARIAL SIZE=1 COLOR=WHITE>" + rs(i).Name + "</FONT></TD>")
        end if
        ls.Close
        set ls=nothing
        ELSE
    ...
    'Response.Write("<TD BGCOLOR=BLUE><FONT FACE=ARIAL SIZE=1 COLOR=WHITE>" + rs(i).Name + "</FONT></TD>")
  next
  Response.Write("<TD BGCOLOR=BLUE><FONT FACE=ARIAL SIZE=1 COLOR=WHITE><B>Tools</B></FONT></TD>")
  ...
  END IF
  IF FEDIT THEN
  Response.Write("<A HREF=FORMEDIT.ASP?DID=" & DID & "&ID=" & rs(3) & "&UID=" & UID & " target=news><img src=update.gif border=0 width=20 alt='Edit Document'></A>")
  END IF
  IF FDEL THEN
...
&#169; Copyright 2001 . All rights reserved. SoftRobot Document Server
