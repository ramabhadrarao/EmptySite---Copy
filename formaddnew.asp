...
'-----------------------------------------
	...
	<%elseif i=4 AND DETID>0 then'--------------------
        %><td><font size=1 face="<%=FNT%>"><img src=required.gif> <%=FLDNM %><%'= rs(i).Type %>: </font><FONT FACE=ARIAL SIZE=1>
  <INPUT type=TEXT READONLY NAME=<%=name%> value=<%=DETID%> size="20" ></FONT></td>
		<%'COLCNT=COLCNT+1%>
		<%
    ...
    elseif rs(i).name = "CALENDERID" then
		...
		<%
    elseif rs(i).name = "UID" then
		%><INPUT type=hidden NAME=<%=name%> value=<%=UID%>>
	    <%
    elseif rs(i).name = "TOTAL" OR rs(i).name = "DONEFLAG" OR rs(i).name = "DONEDATE" then
    ...
    elseif rs(i).name = "ORDERVALUE" then
   	...
   	elseif rs(i).Type = 11 then
		%><td><font size=1 face="<%=FNT%>"><img src=required.gif> <%=FLDNM %><%'= rs(i).Type %></font><br>
  <INPUT TYPE=checkbox NAME=<%=name %> style="background: #E1F2FD; color: #FF00FF" value="on"><br><font face=arial size=1>(boolean only)</font></td> <%
    '-----------------------------------------
    elseif rs(i).Type = 3 then
		...
		<br><INPUT TYPE=TEXT SIZE=<%= size %> style="<%=FNTFAMILY%>" name=<%= name %> value=1>
		...
		%><td><font size=1 face="<%=FNT%>"><img src=required.gif> <%=FLDNM %><%'= rs(i).Type %></font><br><INPUT TYPE=TEXT SIZE=<%=size %> style="<%=FNTFAMILY%>" name=<%= name %> value=0><br><font face=arial size=1>(Int only)</font></td> <%
		end if
		ls.Close
		set ls=nothing
    '-----------------------------------------
    elseif rs(i).Type = 5 or rs(i).Type = 6 or rs(i).Type = 131 or rs(i).Type = 4 then'FLOAT
		%><td><font size=1 face="<%=FNT%>"><img src=required.gif> <%=FLDNM %><%'= rs(i).Type %></font><br><INPUT TYPE=TEXT SIZE=<%=size %> style="<%=FNTFAMILY%>" name=<%=name %> value=0><br><font face=arial size=1>(Float only)</font></td> <%
    elseif rs(i).Type = 17 or rs(i).Type = 2 or rs(i).Type = 128 or rs(i).Type = 204 then'INT
		%><td><font size=1 face="<%=FNT%>"><img src=required.gif> <%=FLDNM %><%'= rs(i).Type %></font><br><INPUT TYPE=TEXT SIZE=<%=size %> style="<%=FNTFAMILY%>"  name=<%=name %> value=0><br><font face=arial size=1>(Integer only)</font></td> <%
    ...
    elseif rs(i).Type = 203 or rs(i).Type = 201 then
		...
		%><td><font size=1 face="<%=FNT%>"><img src=required.gif> <%=FLDNM %><%'= rs(i).Type %></font><br>
  <INPUT TYPE=TEXT  style="<%=FNTFAMILY%>" maxlength=<%=size%> name=<%= name %> size="20" ><br><font face=arial size=1>(Text only)</font></td><%
    end if
    COLCNT=COLCNT+1
 next
%> 
