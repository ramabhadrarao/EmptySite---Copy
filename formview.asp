...
Sub GenerateTable1( rs )
  ...
  for i = 0 to rs.fields.count - 3
    Response.Write("<TD  bordercolordark=#BADFFE bordercolorlight=#808080><font face=arial size=1>" + rs(i).Name + "</font></TD>")
  next
  ' write each row
...
'----------------------
    if i=3 then
    %><tr><td><font size=1 face="<%=FNT%>"><IMG SRC=required.gif> <%=FLDNM %><%'= rs(i).Type %></font></td><td><font face="<%=FNT%>" size=1><%=value%></font></td></tr> <%
    elseif (rs(i).Type = 3) and (value > "0") then
		...
		%><tr><td><font size=1 face="<%=FNT%>"><IMG SRC=required.gif> <%=LISTNAME %><%'= rs(i).Type %></font></td>
		<td>
...
Sub GenerateTable( rs, DDID, Conn )
  ...
  '--------------------------------------
		...
		if not ls.EOF then
		LISTNAME=ls("LISTNAME") 
        Response.Write("<TD bgcolor=#D5EAFF bordercolordark=#FFFFFF bordercolorlight=#808080 align=LEFT><FONT face='" & FNT & "' SIZE=1 >" + LISTNAME + "</FONT></TD>")
        ELSE
  ...
  '--------------------------------------
      ...
      if isnull(v) then v = ""
      '----------------------------
   if (rs(i).Type = 3) and (v > "0") then '-----1
		Set ls=Server.CreateObject("ADODB.Recordset")
		ls.Open "Select * from LISTBOX where DETFLAG=1 AND DDID=" & DDID & " AND COLUMNNO=" & i, Conn
		if not ls.EOF then '---4
		LISTNAME=ls("LISTNAME")
		LISTTABLE=ls("LISTTABLE")
		LISTCOLUMN=ls("LISTCOLUMN")
		LISTVALUE=ls("LISTVALUE")
		FFLAG=ls("LINKFLAG"): IF ISNULL(FFLAG) OR FFLAG="" THEN FFLAG=0
			...
			IF FLAG THEN '---2
			%>
			<TD VALIGN=TOP bgcolor=white align=left ><FONT face="<%=FNT%>" SIZE=1>
		...
		<%
			...
			IF FLAG THEN'---------------3
			Response.Write( "<TD VALIGN=TOP bgcolor=white align=RIGHT><FONT face='" & FNT & "' SIZE=1>" & CStr( v ) & "</FONT></TD>" )
			ELSE
		    Response.Write( "<TD VALIGN=TOP bgcolor=#E1F2FF align=RIGHT><FONT face='" & FNT & "' SIZE=1>" & CStr( v ) & "</FONT></TD>" )
			END IF'---------------------3
		end if'----------------------------4
		ls.Close
		set ls=nothing
    '-----------------------------------------
   		 ...
   		 IF FLAG THEN
			Response.Write( "<TD VALIGN=TOP bgcolor=white align=LEFT><FONT face='" & FNT & "' SIZE=1>" & CStr( v ) & "</FONT></TD>" )
		   ELSE
