...
Response.Write( "<TR>" )
Response.Write( "<TD valign=middle bgcolor=#D4D4D4 bordercolordark=#FFFFFF bordercolor=#808080 bordercolorlight=#808080>&nbsp;&nbsp;<FONT FACE=ARIAL SIZE=2><img src=60-60-60.gif border=0 alt='SoftRobot Document Server'><B> Document Updated</B></FONT></TD>" )
Response.Write( "</TR>" )
...
<%
 ...
 %><%
  ...
  '-------------------------------check errorflag
  IF ERRORFLAG THEN
  Response.Write "<font face=arial size=1>Error: Cannot update data because of error in entering data as mentioned below:(Press your back button for correction)</font>"
  Response.End
  END IF
  rs.Update
  %> 
...
%>
<%SIGN=REQUEST("SIGN")%>
<%if SIGN="" Then Response.Write "<font face=arial size=1>ERROR:SIGN IS BLANK</font>":Response.End%>
<%UID=REQUEST("UID")%>
<%if UID="" Then Response.Write "<font face=arial size=1>ERROR:UID IS NULL</font>":Response.End%>
<%ID=REQUEST("ID")%>
<%if ID="" Then Response.Write "<font face=arial size=1>ERROR:ID IS NULL</font>":Response.End%>
<%DID=REQUEST("DID")%>
<%if DID="" Then Response.Write "<font face=arial size=1>ERROR:DID IS NULL</font>":Response.End%>
<%count=Request("i") 'HOW MANY FIELDS%>
<%if count="" Then Response.Write "<font face=arial size=1>ERROR:count is null</font>":Response.End%>
...
TSQL="INSERT INTO TRACKING (UID, DID, ID, FUNC, REMARKS) VALUES (" & UID & ", " & DID & ", " & ID & ", 'EDIT','" & REMARKS & "')"
'Response.Write "<hr>" & TSQL
Conn.EXECUTE TSQL
'-----------------------ADD IN TRACKING TABLE
ELSE
Response.Write "<font face=arial size=1>Error: Wrong Signature, Please enter correct signature.</font>" 
END IF
Conn.Close
Set Conn=nothing
%>
